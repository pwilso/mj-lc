...
#define PMI_VERSION 150


static const char PMIModels_SccsId[] =
  "$Id: //tcad/gemini/A-2008.09/src/dessis/src/PMIModels.h#1 $, "
  "Copyright Synopsys, Inc. 2000-2008";
...
class PMI_EXTERNAL PMI_Vertex_Interface {
...
public:
  ...
  // step size on seconds during transient simumations
  double ReadTransientStepSize () const;

  // step type during transient simumations
...
public:
  ...
  const NS_PMI_MSC::Vertex_Interface* impl() const;
  NS_PMI_MSC::Vertex_Interface* impl();
  PMI_MSC_Vertex_Interface(const PMI_Environment&, const std::string&,
...
private:
  des_bulk (std::string name, std::string material);
  void add_element (des_element* e) { w_element.push_back (e); }
  void add_regioninterface (des_regioninterface* ri) { w_regioninterface.push_back (ri); }
...
public:
  ...
  size_t NumberOfMSCStates (
    const std::string& msc_name ) const;
  ...
  bool ReadMSCOccupations (
    const std::string& msc_name, const des_region* region,
...
class PMI_EXTERNAL PMI_Device_Interface {
...
public:
  ...
  (double E,                    // transition energy
   ...
   double et,                   // electron temperature if hydro, lattice temperature otherwise
   double ht,                   // hole temperature if hydro, lattice temperature otherwise
   double& dpphasedn) = 0;      // derivative of ... coefficient
                                // with respect to electron density
  ...
  (double E,                    // transition energy
   ...
   double et,                   // electron temperature if hydro, lattice temperature otherwise
   double ht,                   // hole temperature if hydro, lattice temperature otherwise
   double& dpphasedp) = 0;      // derivative of ... coefficient
                                // with respect to hole density
  ...
  (double E,                    // transition energy
   ...
   double et,                   // electron temperature if hydro, lattice temperature otherwise
   double ht,                   // hole temperature if hydro, lattice temperature otherwise
   double& dpphaseet) = 0;      // derivative of ... coefficient
                                // with respect to electron or lattice temperature
  ...
  (double E,                    // transition energy
   ...
   double et,                   // electron temperature if hydro, lattice temperature otherwise
   double ht,                   // hole temperature if hydro, lattice temperature otherwise
   double& dpphaseht) = 0;      // derivative of ... coefficient
                                // with respect to hole or lattice temperature
};
...
class PMI_EXTERNAL PMI_MSTrapCapture : public PMI_Vertex_Interface {
public:
  PMI_MSTrapCapture ( const PMI_Environment& env );
  virtual ~PMI_MSTrapCapture ();

  // methods to be implemented by user
  virtual void Compute_capt (
    const double n,
    const double p,
    const double t,
    const double tn,
...
public:
  ...
  // initialize parameter from Sentaurus Device parameter file or from default value
  // (override for PMI_Vertex_Interface::InitParameter)
  // double InitParameter (const char* name, double defaultvalue, int i=-1, int k=-1) const;
...
typedef PMI_MSTrapCapture* new_PMI_MSTrapCapture_func
  (const PMI_Environment& env, int id);
...
// virtual constructor
PMI_EXTERNAL new_PMI_MSTrapCapture_func new_PMI_MSTrapCapture;
