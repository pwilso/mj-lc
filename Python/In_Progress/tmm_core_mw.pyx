...
### check the conditions on the angle for the front layer (usually air) -pwils
    ...
    if np.imag(n_list[li+1]*cos(th_list[li+1])) >= 0: # added the '=', no flipping needed if component is 0j [2026-06-30 pwils]
        ...
        #print(n_list)
        #print(th_list)
        #print([cos(x) for x in th_list])
        #print([n_list[i]*cos(th_list[i]) for i in range(len(th_list))])
        #print(np.imag(n_list[li+1]*cos(th_list[li+1])))
...
# ###############
    ...
    for i in range(num_layers-1):
        #string = "Getting transmission and reflection coeffcients at layer " + str(i)
        #sys.stdout.write(string)
        try:
...
cdef float calc_a_def_int(int layer,  double yB1, double yB2, Tmm_data_internalsource data): # layer = lk
    ...
    cdef complex power_out2 = data.power_output2
    cdef int pol = data.pol
    cdef complex v_fwd_z1 = 0+0j 
    cdef complex w_fwd_z1 = 0+0j
    cdef complex v_bwd_z1 = 0+0j
    cdef complex w_bwd_z1 = 0+0j
    cdef complex v_fwd = 0+0j 
    cdef complex w_fwd = 0+0j
    cdef complex v_bwd = 0+0j
    cdef complex w_bwd = 0+0j
    cdef complex v_fwd_z2 = 0+0j 
    cdef complex w_fwd_z2 = 0+0j
    cdef complex v_bwd_z2 = 0+0j
    cdef complex w_bwd_z2 = 0+0j
    cdef double z1 = 0.
    cdef double z2 = 0.
    cdef complex A1_fwd_z1, A1_bwd_z1, A2_fwd_z1, A2_bwd_z1
    cdef complex A1_fwd_z2, A1_bwd_z2, A2_fwd_z2, A2_bwd_z2
    cdef complex A3_fwd_z1, A3_fwd_star_z1, A3_bwd_z1, A3_bwd_star_z1
    cdef complex A3_fwd_z2, A3_fwd_star_z2, A3_bwd_z2, A3_bwd_star_z2
    cdef complex A1_fwd, A1_bwd, A2_fwd, A2_bwd
    cdef complex A3_fwd, A3_fwd_star, A3_bwd, A3_bwd_star
    cdef double def_int_fwd, def_int_bwd
    # end = time.time()
    ...
    # if  layer < data.source_layer: # you are to the left of the emission source
        # # start = time.time()
        # v_fwd_z1 = data.vw_list_l1[layer+1][0] # Ef for the forward emission
        # w_fwd_z1 = data.vw_list_l1[layer+1][1] # Eb for the forward emission
        # v_bwd_z1 = data.vw_list_l2[layer+1][0] # Ef for the backward emission
        # w_bwd_z1 = data.vw_list_l2[layer+1][1] # Eb for the backward emission

        # v_fwd_z2 = v_fwd_z1 # z1 and z2 will fall on the same side of the emission point
        # w_fwd_z2 = w_fwd_z1
        # v_bwd_z2 = v_bwd_z1
        # w_bwd_z2 = w_bwd_z1
        ...
        # # print('get vw: ', end-start)
    # elif layer > data.source_layer: # you are to the right of the emission source
        # v_fwd_z1 = data.vw_list_r1[layer - data.source_layer][0] # Ef for the forward emission
        # w_fwd_z1 = data.vw_list_r1[layer - data.source_layer][1] # Eb for the forward emission
        # v_bwd_z1 = data.vw_list_r2[layer - data.source_layer][0] # Ef for the backward emission
        # w_bwd_z1 = data.vw_list_r2[layer - data.source_layer][1] # Eb for the backward emission
    ...
    # elif layer == data.source_layer:
        ...
        # else: # you are to the right of the emission source
            # v_fwd_z1 = data.vw_list_r1[0][0] # Ef for the forward emission
            # w_fwd_z1 = data.vw_list_r1[0][1] # Eb for the forward emission
            # v_bwd_z1 = data.vw_list_r2[0][0] # Ef for the backward emission
            # w_bwd_z1 = data.vw_list_r2[0][1] # Eb for the backward emission
            
            # z1 = yB1 - (sum(data.d_list[1:layer+1]) + data.yA) #distance from boundary to absorption point
            # z2 = yB2 - (sum(data.d_list[1:layer+1]) +  data.yA)

        # if (yB2 > sum(data.d_list[1:layer+1]) + data.yA): # you are to the right of the emission source
            # v_fwd_z2 = data.vw_list_r1[0][0] # Ef for the forward emission
            # w_fwd_z2 = data.vw_list_r1[0][1] # Eb for the forward emission
            # v_bwd_z2 = data.vw_list_r2[0][0] # Ef for the backward emission
            # w_bwd_z2 = data.vw_list_r2[0][1] # Eb for the backward emission
            
            # z1 = yB1 - sum(data.d_list[1:layer+1]) #distance from boundary to absorption point
            # z2 = yB2 - sum(data.d_list[1:layer+1])

        # else: # you are to the left of the emission source
            # v_fwd_z2 = data.vw_list_l1[layer+1][0] # Ef for the forward emission
            # w_fwd_z2 = data.vw_list_l1[layer+1][1] # Eb for the forward emission
            # v_bwd_z2 = data.vw_list_l2[layer+1][0] # Ef for the backward emission
            # w_bwd_z2 = data.vw_list_l2[layer+1][1] # Eb for the backward emission
    ...
    # if pol == TE:#'TE':
        # # start = time.time()
        # A1_fwd_z1 = np.imag(n*cos(th)*kz)*w_fwd_z1*w_fwd_z1.conjugate()/power_out1
        # A1_fwd_z1 = A1_fwd_z1 if np.abs(A1_fwd_z1) > sys.float_info.epsilon else 0.
        # A2_fwd_z1 =np.imag(n*cos(th)*kz)*v_fwd_z1*v_fwd_z1.conjugate()/power_out1
        # A2_fwd_z1 = A2_fwd_z1 if np.abs(A2_fwd_z1) > sys.float_info.epsilon else 0.
        # A3_fwd_z1 = np.imag(n*cos(th)*kz)*v_fwd_z1*w_fwd_z1.conjugate()/power_out1
        # A3_fwd_z1 = A3_fwd_z1 if np.abs(A3_fwd_z1) > sys.float_info.epsilon else 0.
        # A3_fwd_star_z1 = A3_fwd_z1.conjugate()
        # A3_fwd_star_z1 = A3_fwd_star_z1 if np.abs(A3_fwd_star_z1) > sys.float_info.epsilon else 0.
        
        # A1_bwd_z1 = np.imag(n*cos(th)*kz)*w_bwd_z1*w_bwd_z1.conjugate()/power_out2
        # A1_bwd_z1 = A1_bwd_z1 if np.abs(A1_bwd_z1) > sys.float_info.epsilon else 0.
        # A2_bwd_z1 =np.imag(n*cos(th)*kz)*v_bwd_z1*v_bwd_z1.conjugate()/power_out2
        # A2_bwd_z1 = A2_bwd_z1 if np.abs(A2_bwd_z1) > sys.float_info.epsilon else 0.
        # A3_bwd_z1 = np.imag(n*cos(th)*kz)*v_bwd_z1*w_bwd_z1.conjugate()/power_out2
        # A3_bwd_z1 = A3_bwd_z1 if np.abs(A3_bwd_z1) > sys.float_info.epsilon else 0.
        # A3_bwd_star_z1 = A3_bwd_z1.conjugate()
        # A3_bwd_star_z1 = A3_bwd_star_z1 if np.abs(A3_bwd_star_z1) > sys.float_info.epsilon else 0.
        
        # # in the event that z1 and z2 fall on the opposite side of the emission point we calculate values for z1 and z2 separately
        # # although in most cases the two values will end up being the same
        # A1_fwd_z2 = np.imag(n*cos(th)*kz)*w_fwd_z2*w_fwd_z2.conjugate()/power_out1
        # A1_fwd_z2 = A1_fwd_z2 if np.abs(A1_fwd_z2) > sys.float_info.epsilon else 0.
        # A2_fwd_z2 =np.imag(n*cos(th)*kz)*v_fwd_z2*v_fwd_z2.conjugate()/power_out1
        # A2_fwd_z2 = A2_fwd_z2 if np.abs(A2_fwd_z2) > sys.float_info.epsilon else 0.
        # A3_fwd_z2 = np.imag(n*cos(th)*kz)*v_fwd_z2*w_fwd_z2.conjugate()/power_out1
        # A3_fwd_z2 = A3_fwd_z2 if np.abs(A3_fwd_z2) > sys.float_info.epsilon else 0.
        # A3_fwd_star_z2 = A3_fwd_z2.conjugate()
        # A3_fwd_star_z2 = A3_fwd_star_z2 if np.abs(A3_fwd_star_z2) > sys.float_info.epsilon else 0.
        
        # A1_bwd_z2 = np.imag(n*cos(th)*kz)*w_bwd_z2*w_bwd_z2.conjugate()/power_out2
        # A1_bwd_z2 = A1_bwd_z2 if np.abs(A1_bwd_z2) > sys.float_info.epsilon else 0.
        # A2_bwd_z2 =np.imag(n*cos(th)*kz)*v_bwd_z2*v_bwd_z2.conjugate()/power_out2
        # A2_bwd_z2 = A2_bwd_z2 if np.abs(A2_bwd_z2) > sys.float_info.epsilon else 0.
        # A3_bwd_z2 = np.imag(n*cos(th)*kz)*v_bwd_z2*w_bwd_z2.conjugate()/power_out2
        # A3_bwd_z2 = A3_bwd_z2 if np.abs(A3_bwd_z2) > sys.float_info.epsilon else 0.
        # A3_bwd_star_z2 = A3_bwd_z2.conjugate()
        # A3_bwd_star_z2 = A3_bwd_star_z2 if np.abs(A3_bwd_star_z2) > sys.float_info.epsilon else 0.
        # # end = time.time()
        # # print('get A vals: ', end-start)

    # elif pol == TM:#'TM':
        # A1_fwd_z1 = 2*np.imag(kz)*np.real(n*cos(th).conjugate())*w_fwd_z1*w_fwd_z1.conjugate()/power_out1
        # A1_fwd_z1 = A1_fwd_z1 if np.abs(A1_fwd_z1) > sys.float_info.epsilon else 0.
        # A2_fwd_z1 =2*np.imag(kz)*np.real(n*cos(th).conjugate())*v_fwd_z1*v_fwd_z1.conjugate()/power_out1
        # A2_fwd_z1 = A2_fwd_z1 if np.abs(A2_fwd_z1) > sys.float_info.epsilon else 0.
        # A3_fwd_z1 = -2*np.real(kz)*np.imag(n*cos(th).conjugate())*v_fwd_z1*w_fwd_z1.conjugate()/power_out1
        # A3_fwd_z1 = np.imag(n*cos(th)*kz)*v_fwd_z1*w_fwd_z1.conjugate()/power_out1
        # A3_fwd_z1 = A3_fwd_z1 if np.abs(A3_fwd_z1) > sys.float_info.epsilon else 0.
        # A3_fwd_star_z1 = A3_fwd_z1.conjugate()
        # A3_fwd_star_z1 = A3_fwd_star_z1 if np.abs(A3_fwd_star_z1) > sys.float_info.epsilon else 0.

        # A1_bwd_z1 = 2*np.imag(kz)*np.real(n*cos(th).conjugate())*w_bwd_z1*w_bwd_z1.conjugate()/power_out2
        # A1_bwd_z1 = A1_bwd_z1 if np.abs(A1_bwd_z1) > sys.float_info.epsilon else 0.
        # A2_bwd_z1 =2*np.imag(kz)*np.real(n*cos(th).conjugate())*v_bwd_z1*v_bwd_z1.conjugate()/power_out2
        # A2_bwd_z1 = A2_bwd_z1 if np.abs(A2_bwd_z1) > sys.float_info.epsilon else 0.
        # A3_bwd_z1 = -2*np.real(kz)*np.imag(n*cos(th).conjugate())*v_bwd_z1*w_bwd_z1.conjugate()/power_out2
        # A3_bwd_z1 = A3_bwd_z1 if np.abs(A3_bwd_z1) > sys.float_info.epsilon else 0.
        # A3_bwd_star_z1 = A3_bwd_z1.conjugate()
        # A3_bwd_star_z1 = A3_bwd_star_z1 if np.abs(A3_bwd_star_z1) > sys.float_info.epsilon else 0.
        ...
        # # in the event that z1 and z2 fall on the opposite side of the emission point we calculate values for z1 and z2 separately
        # # although in most cases the two values will end up being the same
        # A1_fwd_z2 = 2*np.imag(kz)*np.real(n*cos(th).conjugate())*w_fwd_z2*w_fwd_z2.conjugate()/power_out1
        # A1_fwd_z2 = A1_fwd_z2 if np.abs(A1_fwd_z2) > sys.float_info.epsilon else 0.
        # A2_fwd_z2 =2*np.imag(kz)*np.real(n*cos(th).conjugate())*v_fwd_z2*v_fwd_z2.conjugate()/power_out1
        # A2_fwd_z2 = A2_fwd_z2 if np.abs(A2_fwd_z2) > sys.float_info.epsilon else 0.
        # A3_fwd_z2 = -2*np.real(kz)*np.imag(n*cos(th).conjugate())*v_fwd_z2*w_fwd_z2.conjugate()/power_out1
        # A3_fwd_z2 = np.imag(n*cos(th)*kz)*v_fwd_z2*w_fwd_z2.conjugate()/power_out1
        # A3_fwd_z2 = A3_fwd_z2 if np.abs(A3_fwd_z2) > sys.float_info.epsilon else 0.
        # A3_fwd_star_z2 = A3_fwd_z2.conjugate()
        # A3_fwd_star_z2 = A3_fwd_star_z2 if np.abs(A3_fwd_star_z2) > sys.float_info.epsilon else 0.

        # A1_bwd_z2 = 2*np.imag(kz)*np.real(n*cos(th).conjugate())*w_bwd_z2*w_bwd_z2.conjugate()/power_out2
        # A1_bwd_z2 = A1_bwd_z2 if np.abs(A1_bwd_z2) > sys.float_info.epsilon else 0.
        # A2_bwd_z2 =2*np.imag(kz)*np.real(n*cos(th).conjugate())*v_bwd_z2*v_bwd_z2.conjugate()/power_out2
        # A2_bwd_z2 = A2_bwd_z2 if np.abs(A2_bwd_z2) > sys.float_info.epsilon else 0.
        # A3_bwd_z2 = -2*np.real(kz)*np.imag(n*cos(th).conjugate())*v_bwd_z2*w_bwd_z2.conjugate()/power_out2
        # A3_bwd_z2 = A3_bwd_z2 if np.abs(A3_bwd_z2) > sys.float_info.epsilon else 0.
        # A3_bwd_star_z2 = A3_bwd_z2.conjugate()
        # A3_bwd_star_z2 = A3_bwd_star_z2 if np.abs(A3_bwd_star_z2) > sys.float_info.epsilon else 0.
    ...
    # if np.imag(kz) != 0:
        # # start = time.time()
        # def_int_fwd = 0.5*(A1_fwd_z2*np.exp(2*z2*np.imag(kz))/np.imag(kz) - A2_fwd_z2*exp(-2*z2*np.imag(kz))/np.imag(kz) - 1j*A3_fwd_z2*np.exp(2*1j*z2*np.real(kz))/np.real(kz)  + 1j*A3_fwd_star_z2*exp(-2*1j*z2*np.real(kz))/np.real(kz))
        # def_int_fwd -= 0.5*(A1_fwd_z1*np.exp(2*z1*np.imag(kz))/np.imag(kz) - A2_fwd_z1*exp(-2*z1*np.imag(kz))/np.imag(kz) - 1j*A3_fwd_z1*np.exp(2*1j*z1*np.real(kz))/np.real(kz) + 1j*A3_fwd_star_z1*exp(-2*1j*z1*np.real(kz))/np.real(kz))
        
        # end = time.time()
        # # print('get ints: ', end-start)
        # def_int_bwd = 0.5*(A1_bwd_z2*np.exp(2*z2*np.imag(kz))/np.imag(kz) - A2_bwd_z2*exp(-2*z2*np.imag(kz))/np.imag(kz) - 1j*A3_bwd_z2*np.exp(2*1j*z2*np.real(kz))/np.real(kz)  + 1j*A3_bwd_star_z2*exp(-2*1j*z2*np.real(kz))/np.real(kz))
        # def_int_bwd -= 0.5*(A1_bwd_z1*np.exp(2*z1*np.imag(kz))/np.imag(kz) - A2_bwd_z1*exp(-2*z1*np.imag(kz))/np.imag(kz) - 1j*A3_bwd_z1*np.exp(2*1j*z1*np.real(kz))/np.real(kz)  + 1j*A3_bwd_star_z1*exp(-2*1j*z1*np.real(kz))/np.real(kz))
    ...
    # if math.isnan(def_int_fwd) or math.isinf(def_int_fwd):
        ...
        # print('z1:', z1, ' yB1: ', yB1)
        # print('z2:', z2, ' yB2: ', yB2)
        # print('A1_fwd_z1:', A1_fwd_z1)
        # print('A2_fwd_z1:', A2_fwd_z1)
        # print('A3_fwd_z1:', A3_fwd_z1)
        # print('A3_fwd_star_z1:', A3_fwd_star_z1)
        # print('A1_fwd_z2:', A1_fwd_z2)
        # print('A2_fwd_z2:', A2_fwd_z2)
        # print('A3_fwd_z2:', A3_fwd_z2)
        # print('A3_fwd_star_z2:', A3_fwd_star_z2)
