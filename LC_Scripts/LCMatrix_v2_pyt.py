...
### USER INPUT ################################################################ #!!!
run_method = "CMD_or_TCL" # Updated to be able to input variables using sys.argv from cmd line
                # Useful for running from Sentaurus when the Sentaurus libraries your
                # using don't line up with the gpython version used by the software.
                # Options are now "Py_IDE", "Sentaurus_Python", "CMD_or_TCL"
                # run on the command line (True) or through Sentaurus (False)
...
if run_method == "Sentaurus_Python":
    if "@LC@" == "on": # this is always False from the command line, only relevant if Sentaurus preprocessor has run
      runflag = True # program will run in Sentaurus if not running from command line and LC set to on
      ...
      runflag = False
elif run_method == "CMD_or_TCL":
    if len(sys.argv[0]) > 1:
      runflag = bool(sys.argv[7]) 
      matrix_file = 'LC_Outputs/' + sys.argv[1] + '.csv'
      #specify the name of the file and path to be written
      ...
      #sys.path.append(os.path.join(os.getcwd(), '..', '..', 'Python', 'tmm'))
      print(sys.path)
      
      # these variables are substituted by the sentaurus preprocessor when the script is run through Sentaurus Workbench.
      epi_node = int(sys.argv[2]) 
      sde_node = int(sys.argv[3]) 
      MatPar_node = int(sys.argv[4]) 
      wtot = float(sys.argv[5]) 
      substrate_t = float(sys.argv[6]) 
    else:
      print("Running from CMD line requires passing arguments to the script.\n")
      print("python LCMatrix_pyt.py matrix_filename epi_node sde_node matpar_node wtotm substrate_t run_flag")
else:
  runflag = True
  LC_filepath = './' + matrix_file
...
###############################################################################
          ...
          for lk in range(len(e.layers)):
               ...
               # print(P_arr[:, lk])
               # print(yi_list)
               Ps[lk] = trapz(P_arr[:, lk], x=yi_list)/e.layers[li].thickness ###FIXME: I don't think this works with the new scaled P values (sum * dz again?)
          print('.')
          sys.stdout.flush()
...
if os.path.isfile(LC_filepath):
    ...
    if run_method == "CMD_or_TCL" or "IDE":
        try:
          LC_filepath = './' + matrix_file 
          ...
          print(os.getcwd())
          LC_filepath = '../../../' + matrix_file 
          e = epi_cmd_LC.epifile('../'+ str(epi_node) + '/pp'+ str(epi_node) + '_epi.cmd')
          e.bottom_material(back_mat, material_files_list, back_mat_thickness)
          # Add MatPar data to each layer
          # including n,k vs. wavelength and B_rad
          e.read_parfiles('../'+ str(MatPar_node) + '/n' + str(MatPar_node) + '_mpr.par',
                          angle_list = angles, N_energies = n_energies)
          # creates e.layer.yvalues, a dict indexed by yposition of vertices, by reading the mesh file.
          e.get_layer_vertices('../'+ str(sde_node) + '/n' + str(sde_node) + '_msh.tdr')
    else:
        try:
